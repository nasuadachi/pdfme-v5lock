...
const generate = async (props: GenerateProps): Promise<Uint8Array> => {
  ...
  let pdfDoc: pdfLib.PDFDocument | undefined;

  try {
    const preprocessed = await preprocessing({ template, userPlugins });
    pdfDoc = preprocessed.pdfDoc;
    const { renderObj } = preprocessed;
